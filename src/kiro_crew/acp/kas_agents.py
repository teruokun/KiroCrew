...
def load_agent_spec(agents_dir: Path, agent_id: str) -> dict[str, Any]:
    ...
    too: "cannot verify" is not "not fingerprinted".
    """
    ...
    _require_recorded_fingerprint(agent_id, path, data)
    try:
...
def _require_recorded_fingerprint(agent_id: str, path: Path, data: bytes) -> None:
    """Refuse *data* when *agent_id* is a fingerprinted shared template and the
    bytes do not match the record. See :func:`load_agent_spec`."""
    try:
        recorded = agent_state.get_shared_template_digest(agent_id)
