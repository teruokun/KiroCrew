...
**Project git**: GET `/api/project/git?path=<dir>` (checked-out branch label for the activity panel). `path` is matched against the gateway's own known project directories (every live chat slot's `project` plus the recorded recent-projects list) using pure string normalisation (`expanduser` + `normpath`), with no filesystem access on the untrusted value; an unrecognised directory is refused with 403 plus an SEL audit record, and only the matched server-held string is stat'd. Response is `{"repo": false}`, `{"repo": true, "repoRoot", "branch"}`, or the detached form `{"repo": true, "repoRoot", "detached": true, "head"}`, plus `path` on the envelope. No `git` subprocess is spawned; the branch comes from reading `.git/HEAD` directly (a `git` invocation parses repo-local config, which can carry `[include] path = <file>` and read an arbitrary file, so reading HEAD ourselves removes that class), and a linked worktree's `.git` pointer file (`gitdir: <path>`) is followed to the real HEAD. Both metadata reads go through `hooks.safe_read_prefix` (realpath canonicalisation, sensitive-target rejection, `O_NOFOLLOW`, bounded read). `branch`, `head`, `path`, and `repoRoot` all pass through `redact` before being returned; the SEL audit keeps the real path. Every failure mode degrades to "no branch" rather than erroring, since the only consumer is a decorative label. A detached HEAD gets a fixed 7-char prefix, not git's dynamic abbreviation. GET `/api/project/git/status?path=<dir>` and GET `/api/project/git/log?path=<dir>` use the same known-project boundary and return an explicit `{"repo": false, "files": []}` / `{"repo": false, "commits": []}` when the project directory is not a Git working tree. The status route uses a tri-state, no-follow metadata check before it launches git: missing, malformed, POSIX-symlink, or Windows-reparse markers are confirmed absence without touching marker targets; UNC/device `gitdir:` targets in any separator spelling, and NUL-bearing targets, are rejected lexically before any target probe; on Windows a local-looking gitdir beneath a linked ancestor remains unknown and defers to sandboxed git; unreadable plain markers and inner object-store links likewise remain unknown and defer to sandboxed git; and a structurally valid plain marker is present. A symbolic HEAD is valid only as `ref: refs/...` with no empty edge component, `..`, control character, or whitespace. It rechecks the directory after the subprocess phase. A directory that vanished during process launch or status collection is still absence and returns `200 {"repo": false, "files": []}`; only sandbox, launch, or status failures while the directory still exists return 503 with `code: "git_status_unavailable"`. The Git panel treats only the confirmed `repo: false` response as a terminal no-repository state: it replaces the loading label, hides the clean badge and clean-repository empty copy, and tells the user to select the repository subfolder when one exists.
