...
describe('RemoteCrewPanel', () => {
  ...
  it('labels SSM, confirmed EC2 over SSH, and plain SSH crews accurately', async () => {
    ...
    vi.mocked(api.listInstances).mockResolvedValue({
      active: true,
      warm_set_cap: 10,
      instances: [CLOUD_INSTANCE, ec2Ssh, MANUAL_INSTANCE],
    })
    vi.mocked(api.cloudLaunches).mockResolvedValue({ jobs: [DONE_JOB, ec2SshJob] })
    renderWithProviders(<RemoteCrewPanel />)

    const cloudRow = (await screen.findByText(CLOUD_INSTANCE.name)).closest('[data-crew-id]')
  ...
  it('renames a configured crew and refreshes its visible label', async () => {
    ...
    await u.click(await screen.findByRole('menuitem', { name: 'Rename' }))
    ...
    await waitFor(() => expect(api.updateInstance).toHaveBeenCalledWith('m1', { name: 'Build box' }))
